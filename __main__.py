...
if __name__ == '__main__':
    # vidManager = VideoManager(video_feed_names=['RTSP'],
    #                           streams=['rtsp://wowzaec2demo.streamlock.net/vod/mp4:BigBuckBunny_115k.mov'],
    #                           manual_video_fps=[None], queue_size=2, recording_dir=None,
    #                           reconnect_threshold_sec=1, max_height=1080, method='vlc')
    ...
    for frame_count in itertools.count():
        ...
        for i, video_stream_information in enumerate(vidManager.videos):
            if len(frame_of_each_video_feed[i]) != 0:
                cv2.imshow(video_stream_information['video_feed_name'], frame_of_each_video_feed[i])
